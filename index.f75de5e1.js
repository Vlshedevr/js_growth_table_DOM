...
function deletRow() {
    const curentTr = tBody.querySelectorAll("tr").length;
    if (curentTr > MinTrTd) tBody.lastElementChild.remove();
    disBtnTr();
}
function addColumn() {
    const curentTd = tBody.querySelector("tr").querySelectorAll("td").length;
...
mainCon.addEventListener("click", (e)=>{
    if (e.target === BtnAdAp) addRow();
    if (e.target === BtnDelLeft) deletRow();
    if (e.target === BtnAdLeft) addColumn();
    if (e.target === BtnDelAP) deleteColumn();
});
